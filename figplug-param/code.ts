const icons = [
  { name: "menu",
    svg: '<svg width="$size$" height="$size$" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 18H21V16H3V18ZM3 13H21V11H3V13ZM3 6V8H21V6H3Z" fill="$color$"/></svg>' },
  { name: "settings",
...
const cachedThumbnails = icons.reduce((a, icon) => {
  return { ...a, [icon.name]: processSVG({icon: icon.svg, color: 'black', size: '40'})}
}, {})

function processSVG({icon, color, size}: {icon: string, color: string, size: string}): string {
  ...
  switch (key) {
    case 'icon':
      result.setSuggestions(icons
        ...
        .map(s => ({ name: s.name, data: s.svg, icon: cachedThumbnails[s.name as keyof typeof cachedThumbnails] })))
      break
    case 'size':
      {
...
function startPluginWithParameters(parameters: ParameterValues) {
  ...
  const _processedSvg = icon.replace(/\$size\$/g, size).replace(/\$color\$/g, color)
